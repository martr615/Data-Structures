...
//Subscript operator
int &HashTable::operator[](string key){             //A[jord]

    pair<int, int> pos = findPosition(key);         //pos= (-1, 5)

    if (pos.first == NOT_FOUND) {

        Item *temp = new Item(key);

        tryInsert(temp, pos.second);                //tryInsert(Ett item, 5)

        //rehash if to much space used
        if (loadFactor() > MAX_LOAD_FACTOR){
            reHash();
        }
        pos = findPosition(key);                    //pos = (5, 5)
    }
...
{
    //Ny tabell med dubbla storleken.
    ...
    for(int i = 0; i < tempSize; i++){
        if(tempTable[i] == nullptr){    //Om det är null på den gamla tabellen så skipper vi för vi vill sätta in de slots som har värden.
            continue;
        }
        else if(tempTable[i] != nullptr){
...
//private function to insert new item at the position given by the hash function.
void HashTable::tryInsert(Item *anItem, int pos){
    if (pos > size){
        tryInsert(anItem, 0);
    }
    ...
    else{   //Om det är upptaget index, kör rekursivt (med pos + 1) tills hittar ett tomt index.
        tryInsert(anItem, pos + 1);
    }
}
...
pair<int, int> HashTable::findPosition(string key) const {
    ...
    pair<int, int> position(NOT_FOUND, NOT_FOUND);
    bool flipped = false;
    ...
    for (int i = h(key, size); i < size; ++i) {
        ...
        if (!hTable[i]){
            ...
            //key found.
            position.first = position.second = i;
            break;
        }
