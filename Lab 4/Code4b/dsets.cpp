...
void DSets::join(int r, int s)
{
    ...
    // simple union, från s.359 i datorstrukturboken.
    if (array[r] > array[s]){   //array[r] är mindre, så sätt den som ny rot.

        array[s] += array[r]; //nykod
        array[r] = s; //riktar om.
    }
    else{

        array[r] += array[s]; //nykod
        array[s] = r;
    }
}
...
int DSets::find(int x)
{
    ...
    // simple find, från s.361 i datorstrukturboken.
    if (array[x] < 0)
    {
        return x;
    }
    ...
    {
        return array[x] = find(array[x]); //rekursivt.
    }
